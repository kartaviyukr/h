...
> Текущая стадия: **Фаза 4** — RAG по рецептам: корпус рецептов в pgvector с
> локальными эмбеддингами, retrieval «приземляет» генерацию меню (DeepSeek
> использует реальные рецепты как основу, с graceful-fallback на чистую
> генерацию). Ранее: Фаза 2 — auth/профиль, Фаза 3 — генерация меню. Сравнение
> цен, корзина и фронтенд-экраны — последующие фазы.
